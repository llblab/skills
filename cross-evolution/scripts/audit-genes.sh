#!/usr/bin/env bash
# audit-genes — Gene × Skill matrix with fitness scoring and gene discovery.
...
has_skill_definitions() {
  local root=$1
  [[ -d "$root" ]] || return 1
  find "$root" -type f -name "SKILL.md" ! -path "*/.git/*" -print -quit 2>/dev/null | grep -q .
}

select_default_skills_dir() {
  local candidate
  for candidate in "$DEFAULT_SKILLS_DIR" "$AGENTS_SKILLS_DIR" "$PWD_SKILLS_DIR" "$LOCAL_SKILLS_DIR"; do
    if has_skill_definitions "$candidate"; then
      printf '%s\n' "$candidate"
      return 0
    fi
  done
...
Defaults:
  skills_dir = first directory with SKILL.md files among $SKILLS_HOME (or ~/.skills), ~/.agents/skills,
               current working directory, then sibling skills directory next to cross-evolution.

Environment:
  NO_COLOR=1            Disable ANSI colors for CI/log processing.
EOF
}
...
if [[ -z "$SKILLS_DIR" ]]; then
  SKILLS_DIR="$(select_default_skills_dir)"
fi
...
append_proposed_gene() {
  ...
  ' "$GENES_FILE" > "$tmp_file"; then
    rm -f "$tmp_file"
    echo "  ${YELLOW}⚠ Could not append proposed gene '$gid' (Proposed section not found).${NC}"
    return 1
  fi
...
for skill in "${SKILLS[@]}"; do
  ...
  for gid in "${GENE_IDS[@]}"; do
    if [[ "${DETECTED[$skill:$gid]}" == "1" ]]; then
      printf "  ${GREEN}✓ %-$((COL_WIDTH - 2))s${NC}" "${GENE_NAMES[$gid]}"
    else
      printf "  ${RED}✗ %-$((COL_WIDTH - 2))s${NC}" "${GENE_NAMES[$gid]}"
    fi
  done
  echo
done
  ...
  for i in "${!CONFLICT_A[@]}"; do
    ...
    for skill in "${SKILLS[@]}"; do
      if [[ "${DETECTED[$skill:$ca]:-0}" == "1" && "${DETECTED[$skill:$cb]:-0}" == "1" ]]; then
        ...
        echo -e "  ${YELLOW}⚡ ${skill}: ${ca} × ${cb}${NC} — ${CONFLICT_DESC[$i]}"
...
for gid in "${GENE_IDS[@]}"; do
  ...
  if [[ $carriers -eq 0 ]]; then
    echo -e "  ${YELLOW}☠ ${gid}${NC} — carried by 0/${#SKILLS[@]} skills (candidate for deprecation)"
...
for skill in "${SKILLS[@]}"; do
  ...
  for gid in "${GENE_IDS[@]}"; do
    ...
    case "$scope" in
      all)
        echo "  → $skill: ${GENE_NAMES[$gid]} ($gid, weight=${GENE_WEIGHT[$gid]})"
        RECS=$((RECS + 1))
        ;;
      has_scripts)
        if $has_scripts; then
...
for gid in "${DISCOVER_IDS[@]}"; do
  ...
  if [[ $carriers -ge ${DISCOVER_MIN_CARRIERS[$gid]} ]]; then
    DISCOVERED_IDS+=("$gid")
    echo "  + Candidate '$gid' (${DISCOVER_NAME[$gid]}) — carriers: $carriers/${#SKILLS[@]}"
  fi
done
